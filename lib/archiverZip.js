...
module.exports = async function(localpath = path.join(process.cwd(), 'uploads'), category = 'test') {
  const archiverZip = () => new Promise((resolve, reject) => {
    ...
    // zip文件名
    const zipfilename = moment().format('YYYYMMDD_HHmmss_SSS') + '.zip';

    const output = fs.createWriteStream(path.join(deployedDir, zipfilename));
